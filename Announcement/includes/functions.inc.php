...
{
    // Code snippets for class id fetching.
    $sql = "SELECT Class_ID FROM class_course  WHERE F_ID = ? AND Course_ID = ?;";
    ...
    mysqli_stmt_bind_param($stmt, "ss", $_SESSION["id"], $course_id);
    mysqli_stmt_execute($stmt);
    ...
    // mysqli_stmt_bind_param($stmt, "ssss", $course_id, $class_id, $title, $desc);
    // mysqli_stmt_execute($stmt);
    // mysqli_stmt_close($stmt);  
